            ...
            <CardContent className="p-0">
               <div className="space-y-4">
                  {data.map((item, index) => {
                     ...
                     const maxWidth = 100;
                     const patientWidth =
                        (item.value / maxScaleValue) * maxWidth;
                     const refMinWidth =
                        (item.refMin / maxScaleValue) * maxWidth;
                     const refMaxWidth =
                        (item.refMax / maxScaleValue) * maxWidth;
                     const prevTestPosition = item.prevValue
                        ? (item.prevValue / maxScaleValue) * maxWidth
                        : refMinWidth;

                     return (
                        <AccordionItem value={item.labelMn} key={index}>
                           <AccordionTrigger>
                              <div className="w-full py-4 px-3 bg-white grid grid-cols-[1fr_1fr_1fr] sm:grid-cols-[0.5fr_0.8fr_0.5fr_1.8fr] items-center gap-2 rounded-lg shadow-md">
                                 ...
                                 <div className="flex flex-col items-start">
                                    <span className="text-sm text-gray-700">
                                       Хэвийн хэмжээ {item.refMin} -{" "}
                                       {item.refMax} {item.unit}
                                    </span>
                                 </div>
