        ...
        <div className="flex flex-col items-center mr-10 mt-4">
          {[1, 2, 3, 4].map((s, i) => (
            <div key={s} className="flex flex-col items-center">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center text-xl font-semibold border transition-colors duration-500 ${step === s
              ...
              {i < totalSteps - 1 && (
                <div
                  className={`w-px h-40 transition-all duration-500 ease-in-out ${step > s ? "bg-blue-500" : "bg-gray-300"
                    }`}
                />
              )}
            </div>
